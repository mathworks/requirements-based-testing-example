function  importXLS2SLT( inputFileName, testCase )
% IMPORTXLS2SLT reads a test case spreadsheet into the Test Mgr

    [testPath, ~, ~] = fileparts(inputFileName);
    if isempty(testPath)
    ...
    [modelName, dtTbl, mdlSampleTime] = setupModel(modelName,harnessName);

    % clear existing test case
    clearTestCase(testCase);
        
    iteration(tcMax) = sltest.testmanager.TestIteration;
    for tcNum = 1:tcMax

        [rawT] = getWorksheet(inputFileName,tcNames{tcNum});
        [testCase, iteration(tcNum)] = addIterationStd(testCase, ...
            testPath,['case' num2str(tcNum,'%02d') '_' tcNames{tcNum}], ...
            rawT, dtTbl, mdlSampleTime);

    end

    for i=1:numel(iteration)
        testCase.addIteration(iteration(i),iteration(i).Name);
    end
...
function [testCase, iteration] = addIterationStd(testCase, testPath, ...
    ...
    %commCol     = findColumnGroups(row1,'Comments',lastCol);
    ...
    if ~isempty(inputCol)
        ...
        %   -- map in testMgr
        testInput = testCase.addInput(fullfile(testPath,inputMatFileName),1);

        % addInput automatically creates an iteration, so delete
        iter2remove = testCase.getIterations;
        testCase.deleteIterations(iter2remove);
    ...
    if ~isempty(outputCol)
        ...
        testBaseline = testCase.addBaselineCriteria(fullfile(testPath,outputMatFileName),true);
