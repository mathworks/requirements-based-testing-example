function BTCadaptor(testCase, inputFileName)  
...
function [testCase, iteration] = addIterationStd(testCase, testPath, ...
    ...
    rowStart = find(cellfun(@(x) strcmpi(x,'mode'), rawT(:,1)), 1);
    if isempty(rowStart)
       ME = MException('GenericAdaptor:testDataStartRowNotFound', ...
            'Error:  "Step" not found in first column of raw spreadsheet data for test case "%s"', ...
            iterName);
       throw(ME)
    end
    ...
    calCol      = findColumnGroups(row1,'Parameters',lastCol);
    ...
    if ~isempty(inputCol)
        ...
        for k=1:size(inputValues,2)
            ...
            testData = addTimeSeries(testData, inputNames{k}, sigValues, ...
                mdlSampleTime, dtTbl, timeInput, 1);
    ...
    if ~isempty(outputCol)
        ...
        for k=outputSignalCol
            ...
            testData = addTimeSeries(testData, outputNames{(k-1)/2 + 1}, ...
                sigValues, mdlSampleTime, dtTbl, timeOutput, 1);
...
function testData = addTimeSeries(testData, sigName, sigValues, ...
    ...
    [values] = castData(dataType,sigValues);
...
%--------------------------------------------------------------------------
function [values] = castData(dataType, values) %#ok<INUSD>
    ...
    ME = MException('GenericAdaptor:unableToResolveDataType', ...
        'Error:  Unable to resolve "%s" data type for conversion.', ...
        dataType);
    throw(ME)
